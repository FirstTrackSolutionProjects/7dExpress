...
const generateMonthlyInvoiceHtml = (data) => {
  ...
  const companyName = escapeHtml(from.NAME || '7D Express')
  const companyShort = escapeHtml((from.NAME || '7D Express').split(' ').map(w => w[0]).join('').slice(0,4).toUpperCase())
...
<head>
  ...
  .logo{width:84px;height:84px;border-radius:6px;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#fff;border:1px solid #eee}
