...
const TransactionHistory = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!admin) delete queryParams.merchant_email;

      const query = new URLSearchParams(queryParams).toString();
      const endpoint = admin ? 'admin' : 'merchant';
      const res = await fetch(`${API_URL}/wallet/transactions/${endpoint}?${query}`, {
  ...
  const handleDownload = async () => {
    try {
      const res = await fetch(`${API_URL}/wallet/report/download/${admin ? 'all' : ''}`, {
        method: 'POST',
        headers: { 
    ...
    <div className='py-10 w-full flex flex-col items-center bg-gray-50 min-h-screen'>
      <div className='w-full max-w-7xl px-4 flex flex-col gap-4 mt-8'>
        ...
        <div className='bg-blue-600 text-white p-4 rounded-lg flex flex-wrap gap-3 items-center shadow-md'>
            ...
            {admin && <input name='merchant_email' value={filters.merchant_email} onChange={handleFilterChange} placeholder='Merchant Email' className='p-2 rounded text-black bg-white flex-1 min-w-[200px] focus:outline-none'/>}
